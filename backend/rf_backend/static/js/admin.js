  ...
  recipes.forEach(recipe => {
        ...
        recipeItem.innerHTML = `
            <div class="recipe-info">
                <img src="/static/images/${recipe.image}" alt="${recipe.name}" class="recipe-thumbnail">
                <h3>${recipe.name}</h3>
            </div>
...
function editRecipe(recipeName) {
    ...
    if (recipe && recipe.id) {
        console.log('Editing recipe:', recipeName, 'with ID:', recipe.id);
        localStorage.setItem('editRecipe', recipeName);
        localStorage.setItem('editRecipeId', recipe.id);
        const editUrl = '/edit-recipe/?id=' + recipe.id;
